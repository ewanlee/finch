...
class NMF:
    def __init__(self, n_user, n_item, n_hidden=100, lamda=0.01, sess=tf.Session()):
        """
        Parameters:
        ------------
        n_user: int
        ...
        n_item: int
            Dimensions of item (colume of rating)
        n_hidden: int
            Number of hidden dimensions during factorization
        lamda: int
            Penalty parameter Lambda of the erro term
        sess: object
            tf.Session() object
        stateful: boolean
            If true, the final state for each batch will be used as
            the initial state for the next batch
        """
        self.n_user = n_user
        self.n_item = n_item
