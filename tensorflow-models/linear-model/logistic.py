...
class Logistic:
    ...
    def add_output_layer(self):
        ...
        self.b = self.call_b('b', [self.n_out])
    ...
    def add_backward_path(self):
        ...
        self.acc = tf.reduce_mean(
            tf.cast(tf.equal(tf.argmax(self.logits, 1), self.Y), tf.float32))
        ...
        for epoch in range(n_epoch):
            for X_batch, Y_batch in zip(self.gen_batch(X, batch_size),
                ...
                _, loss, acc = self.sess.run(
                    [self.train_op, self.loss, self.acc], {self.X:X_batch, self.Y:Y_batch})
            val_loss_list, val_acc_list = [], []
            for X_test_batch, Y_test_batch in zip(self.gen_batch(val_data[0], batch_size),
                self.gen_batch(val_data[1], batch_size)):
                v_loss, v_acc = self.sess.run(
                    [self.loss, self.acc], {self.X:X_test_batch, self.Y:Y_test_batch})
