...
class RNNClassifier:
    def __init__(
        self, n_in, n_seq, n_out,
        cell_size=128, n_layer=1, stateful=False, sess=tf.Session()):
        """
        Parameters:
        -----------
        n_in: int
    ...
    def add_lstm_cells(self):
        def cell():
            cell = tf.nn.rnn_cell.LSTMCell(
                self.cell_size, initializer=tf.orthogonal_initializer())
    ...
    def fit(self, X, Y, val_data=None, n_epoch=10, batch_size=128, en_exp_decay=True, en_shuffle=True,
        ...
        for epoch in range(n_epoch): # batch training
            ...
            if val_data is not None: # go through testing data, average validation loss and accuracy
