...
class LinearSVMClassifier:
    ...
    def add_backward_path(self):
        ...
        hinge_loss = tf.reduce_sum(
            tf.maximum(tf.zeros([self.batch_size, 1]), 1 - self.Y * self.logits))
        ...
        for epoch in range(n_epoch):
            for X_batch, Y_batch in zip(self.gen_batch(X, batch_size),
                ...
                _, loss, acc = self.sess.run(
                    ...
                    {self.X:X_batch, self.Y:Y_batch, self.batch_size:len(X_batch)})
            val_loss_list, val_acc_list = [], []
            for X_test_batch, Y_test_batch in zip(self.gen_batch(val_data[0], batch_size),
                self.gen_batch(val_data[1], batch_size)):
                v_loss, v_acc = self.sess.run(
                    [self.loss, self.acc],
                    {self.X:X_test_batch, self.Y:Y_test_batch, self.batch_size:len(X_test_batch)})
    ...
    def predict(self, X_test, batch_size=100):
        ...
        for X_test_batch in self.gen_batch(X_test, batch_size):
            batch_pred = self.sess.run(
                self.pred, {self.X:X_test_batch, self.batch_size:len(X_test_batch)})
    ...
    def call_b(self, name, shape):
        return tf.get_variable(
            name, shape, tf.float32, tf.constant_initializer(0.01))
    # end method _b
